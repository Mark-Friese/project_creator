...
def create_project_structure(project_location, project_name, project_type, additional_packages=None):
    ...
    try:
        ...
        requirements = {
            ...
            "full_stack_app": ["FastAPI", "uvicorn", "pytest"]
        }
        ...
        subprocess.run(
            [f"python", "-m", "venv", os.path.join(project_path, "venv")], check=True)

        # Activate the virtual environment and install base requirements
        subprocess.run([os.path.join(project_path, "venv", "Scripts",
                       "pip"), "install"] + base_requirements, check=True)

        # Install additional packages if provided
        if additional_packages:
            subprocess.run([os.path.join(project_path, "venv", "Scripts",
                           "pip"), "install"] + additional_packages, check=True)
